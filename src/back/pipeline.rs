...
impl TracerPipeline {
    pub unsafe fn new(
        ...
        let (images, image_views, image_allocations) =
            Self::create_images(&mut allocator.lock().unwrap(), device, viewport)
    ...
    unsafe fn create_images(
        ...
        device: &Device,
        viewport: glam::UVec2,
    ) -> anyhow::Result<(Vec<vk::Image>, Vec<vk::ImageView>, Vec<Allocation>)> {
        ...
        Ok((images, image_views, image_allocations))
    }
